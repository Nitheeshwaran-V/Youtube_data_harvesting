...
st.markdown("### :green[YOUTUBE DATA HARVESTING AND WAREHOUSING]")
with st.sidebar:    
    with st.popover(":green[CONNECT TO DATABASE]"):
        ...
        if st.button("RELOAD MYSQL"):
            Table = tables()
            st.success(Table)
    
    st.divider()

    st.markdown("**:red[CHANNEL NAMES IN MONGODB]**")
    ch_nam = []
    db = client["youtube_data"]
    coll = db["channel_details"]
    for ch_data in coll.find({},{"_id":0, "channel_info":1}):
        ch_nam.append(ch_data["channel_info"]["Channel_Name"])
    
    dataf = pd.DataFrame(ch_nam, columns = ["CHANNEL NAMES"])
    st.table(dataf)



show_table = st.selectbox(":red[TABLES]", 
                         ("CHANNELS", "PLAYLISTS", "VIDEOS", "COMMENTS"),
                         index = None,
                         placeholder = "Select the table to view")
st.write(":blue[Selected Table]", show_table)
...
question = st.selectbox(":red[QUESTIONS]", ("1. Names of all the videos and their channels name",
                                                ...
                                                index = None,
                                                placeholder = "Select any of the question")
st.write(":blue[Selected Question]", question)
